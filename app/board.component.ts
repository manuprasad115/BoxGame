...
export class BoardComponent implements OnInit {
    ...
    chainCount(_edge: Edge, _bid: number): number {
        if (_edge != null) {
            ...
            if (_bid == 0) {
                _boxes = this.boxListByEdgeId(_edge.id).filter(b => b.getBlankEdges().length == 2);
            }
            else {
                _boxes = this.boxListByEdgeId(_edge.id).filter(b => b.getBlankEdges().length == 2 && b.id != _bid);
            }
